...
DATA = b"\xa5\x39\x0b\x00\x00\x00\x00\x00" \
...
class EchoRequest:
    def __init__(self, data=DATA, custom_id=None):
    ...
    def __bytes__(self):
        header_without_checksum = struct.pack('bbHHh', 8, 0, 0, self.id, 1)
        request_without_checksum = header_without_checksum + self.data
        checksum = self.get_checksum(request_without_checksum)
        header = struct.pack('bbHHh', 8, 0, socket.htons(checksum), self.id, 1)
